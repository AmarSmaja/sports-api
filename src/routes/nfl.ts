...
import { fetchNflEvents, mapEspnEventToUnified } from "../lib/espn";
...
export const nflRoutes: FastifyPluginAsync = async (app) => {
  app.get("/games", async (req, reply) => {
    ...
    const events = await fetchNflEvents(effectiveDate);
    const data = events.map((e) => mapEspnEventToUnified(e, 'nfl', effectiveDate));
