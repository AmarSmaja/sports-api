...
const SUPPORTED = ["nba", "nfl", "football", "cbb", "cfb", "mma"] as const;
...
function isSupportedSport(s: string): s is SupportedSport {
  ...
  app.get(
    ...
    async (req, reply) => {
      ...
      if (!isSupportedSport(sport)) {
        return reply.status(400).send({ error: "Unsupported sport", sport, supported: SUPPORTED });
      }
      ...
      const targetPath = qs ? `/${sport}/games?${qs}` : `/${sport}/games`;
